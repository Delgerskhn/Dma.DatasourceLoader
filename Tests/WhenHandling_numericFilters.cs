...
namespace Tests.DatasourceLoader
{
    public class WhenHandling_numericFilters
    {
        ...
        public void ShouldApply_equalFilter()
        {
            ...
            var res = new NumericFilter(filter).ApplyFilter<SampleData>(source.AsQueryable());
        ...
        {
            var filter = new FilterCriteria { 
                DataType = DataSourceType.Numeric, FilterType = FilterType.LessThanOrEqual,
                NumericValue = 25, FieldName = nameof(SampleData.IntProperty) };
            var res = new NumericFilter(filter).ApplyFilter<SampleData>(source.AsQueryable());
        ...
        {
            var filter = new FilterCriteria { 
                DataType = DataSourceType.Numeric, FilterType = FilterType.GreaterThan,
                NumericValue = 25, FieldName = nameof(SampleData.IntProperty) };
            var res = new NumericFilter(filter).ApplyFilter<SampleData>(source.AsQueryable());
        ...
        {
            var filter = new FilterCriteria {   
                DataType = DataSourceType.Numeric, FilterType = FilterType.GreaterThanOrEqual,
                NumericValue = 25, FieldName = nameof(SampleData.IntProperty) };
            var res = new NumericFilter(filter).ApplyFilter<SampleData>(source.AsQueryable());
